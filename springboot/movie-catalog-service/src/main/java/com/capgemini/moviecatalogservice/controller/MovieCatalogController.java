...
public class MovieCatalogController {
	...
	public MovieCatalog getMovieCatalog(@PathVariable String userId) {
		UserRating ratings= restTemplate.getForObject("http://localhost:8085/ratings/" + userId, UserRating.class);
		...
		for(Rating movieRating: movieRatings) {
			...
			catalogItem.add(new CatalogItem(movie, movieRating.getRating()));
			
		}
		MovieCatalog movieCatalog=new MovieCatalog(catalogItem);
		return movieCatalog;
	}
}
